	...
	const select = async (songId: string, candidateId: string) => {
		...
		try {
			...
			const next = songs.find(
				(s) => s.id !== songId && !s.selected_candidate_id,
			);
		...
		<section className="flex flex-col gap-6">
			<div className="flex flex-col gap-1">
				...
				<p className="text-sm text-muted-foreground">
					One source per song. Thumbnails help you judge framing before download.
				</p>
			</div>
			...
			>
				...
				{songs.map((song) => {
					...
					return (
						<TabsContent key={song.id} value={song.id} className="mt-0 flex flex-col gap-4">
											...
											<li key={c.id} className="flex min-h-0">
												...
												>
													...
													<div className="flex flex-1 flex-col gap-1.5 p-3">
														<span
															className="line-clamp-2 h-10 text-sm font-medium leading-5"
															title={c.title}
														>
															{c.title}
														</span>
