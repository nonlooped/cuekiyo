...
export function RenderOrder({
  ...
  useEffect(() => {
    api
      .listSongs(projectId)
      .then(async (s) => {
        setSongs(s.toSorted((a, b) => a.render_order - b.render_order))
        const entries = await Promise.all(
          s.map(async (song) => {
            const cands = await api.listCandidates(projectId, song.id)
            const selected = cands.find((c) => c.is_selected)
            return [song.id, selected?.view_count ?? 0] as const
          })
        )
        setViewMap(Object.fromEntries(entries))
      })
  ...
  const autoSort = () => {
    ...
    setSongs(
      songs.toSorted((a, b) => (viewMap[b.id] ?? 0) - (viewMap[a.id] ?? 0))
    )
    setSortKey((k) => k + 1)
  }
  ...
  const confirm = () => {
    ...
    api
      ...
      .then(() => api.confirmRenderOrder(projectId))
      .then(() => {
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {songs.length} clips will be combined. This can take several
