...
export default function PipelineTimeline({ status }: { status: ProjectStatus }) {
      ...
      <div className="flex md:hidden items-center gap-3 px-1 py-2">
        ...
        >
          {running ? (
            ...
            <span className="text-xs font-medium text-lime">{activeIndex + 1}</span>
          )}
        </div>
      ...
      <div className="hidden md:block overflow-x-auto [&::-webkit-scrollbar]:hidden [scrollbar-width:none]">
        ...
        >
          {PIPELINE_STAGES.map((stage, index) => {
            ...
            const stateLabel = isComplete ? "Completed" : isActive ? "Current" : "Upcoming";
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col items-center gap-1.5 flex-shrink-0">
                  ...
                  >
                    {isComplete && (
                      <Check size={12} className="text-muted" strokeWidth={2.5} />
                    )}
                    {isActive && running && (
                      <Loader2 size={13} className="animate-spin text-lime" />
                    )}
